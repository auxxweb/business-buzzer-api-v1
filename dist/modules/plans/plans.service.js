...
const updatePlan = async (planId, planData) => {
    ...
    if (planData?.plan && planExist?.plan !== planData?.plan) {
        ...
        if (planNameExists) {
            return await generateAPIError(errorMessages?.planNameExists(planData?.plan), 400);
        }
    }
    if (planData?.isDeleted) {
        ...
        if (isBusinessExists > 0) {
            return await generateAPIError(errorMessages?.businessExistsInPlan(isBusinessExists, planExist?.plan), 400);
        }
    }
