...
const updateCategory = async (categoryId, categoryData) => {
    ...
    if (categoryData?.name && categoryExists?.name !== categoryData?.name) {
        ...
        if (catNameExists) {
            return await generateAPIError(errorMessages.categoryNameExists(categoryData?.name), 400);
        }
    }
    if (categoryData?.isDeleted === true || categoryData?.isDeleted === "true") {
        ...
        if (isBusinessExists > 0) {
            return await generateAPIError(errorMessages.businessExistsInCategory(isBusinessExists, categoryExists?.name), 400);
        }
    }
